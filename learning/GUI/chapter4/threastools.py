...
def startThread(action, args, context, onExit, onFail, onProgress=None):
    thread = Thread(target=threaded,args=(action, args, context, onExit, onFail
                                      , onProgress))
    thread.start()
...
class ThreadCount:
    def __init__(self):
        self.count = 0
        self.mutex =  threading.Semaphore     # or use __thread.allocate_lock()
    ...
    def __len__(self): 
        return self.count # True/False if used as a flag
